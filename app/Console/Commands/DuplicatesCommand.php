...
class DuplicatesCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'duplicates:search';
    ...
    public function handle()
    {
        ...
        $counterOfCount = GeneralData::orderBy('id', 'desc')->where('id', '<=', 2271259575)->count();
        $text = "Предположительное колво итераций: $counterOfCount";
        $this->info($text);
        Log::build([
            'driver' => 'single',
            'path' => storage_path('logs/searchDuplicates_stream1.log'),
        ])->info($text);

        GeneralData::orderBy('id', 'desc')->where('id', '<=', 2271259575)->chunk(1000, function($items) use(&$foreachCounter, &$timerStart, &$counterOfCount){

            foreach ($items as $item) {
                $foreachCounter++;

                $text = $foreachCounter . ') Начинаем сверять запись ID: '. $item->id .' от ' . date("d-m-Y", $item->ad_published) . ' (memory usage: ' . memory_get_usage() / 1048576 . ' mb)';
                $this->info($text);
                Log::build([
                    'driver' => 'single',
                    'path' => storage_path('logs/searchDuplicates_stream1.log'),
                ...
                $duplicatesData = GeneralData::where('city', $item->city)
                    ...
                    ->where('ad_published', '>=', $item->ad_published)
                    ->get();
