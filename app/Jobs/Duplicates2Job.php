...
class Duplicates2Job implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $counterOfCount = GeneralData::where('id', '<=', 1893536893)->count();
        $text = "Предположительное колво итераций: $counterOfCount";
        //$this->info($text);
        Log::build([
            'driver' => 'single',
            'path' => storage_path('logs/searchDuplicates_stream-rabbit2.log'),
        ])->info($text);

        GeneralData::where('id', '<=', 1893536893)->chunk(1000, function($items) use(&$foreachCounter, &$timerStart, &$counterOfCount){
